...
import org.andali.schoolreportsweb.model.StudentMark;
import org.andali.schoolreportsweb.model.enums.ExamType;
import org.andali.schoolreportsweb.model.enums.Term;
import org.andali.schoolreportsweb.repository.MarksheetRepository;
...
public class MarksheetService {
    ...
    public void resolveAllGrades(Marksheet marksheet) {
        GradingScale scale = marksheet.getGradingScaleOverride() != null
                ? marksheet.getGradingScaleOverride()
                : marksheet.getSchoolClass().getDefaultGradingScale();

        if (scale == null) throw new IllegalStateException("No grading scale configured for this marksheet or class");
        scale = gradingScaleService.getGradingScaleById(scale.getId());
